Setup = function(){
  if(file.exists('eez2013/temp/referencePoints.csv')){file.remove(file.path(wd,'temp/referencePoints.csv'))}
  referencePoints <- data.frame(goal=as.character(),
                                method = as.character(),
                                reference_point = as.character())
  write.csv(referencePoints, file.path(wd,'temp/referencePoints.csv'), row.names=FALSE)
}
...
b <- b %>%
  ...
  write.csv(gap_fill_data, file.path(wd,'temp/FIS_summary_gf.csv'), row.names=FALSE)
...
NP <- function(scores, layers, status_year, debug = FALSE){
  ...
  np_calc_exposure <- function(np_harvest, hab_extent, FIS_status) {
    ...
    write.csv(gap_fill, file.path(wd,'temp/NP_exposure_gapfill.csv'), row.names=FALSE)
